...
diffcox<-coxph(Surv(SurvivalTimeMonth, Status)~Morphology.Description, data = df)
summary(diffcox)
...
explanatory<-c('AgeAtDiagnosis', 'Gender', 'Morphology.Description',
               'PrimaryTumourSite')
...
coxmodel<-coxph(Surv(SurvivalTimeMonth, StatusCancer==1)~AgeAtDiagnosis+Gender+
                  Morphology.Description+PrimaryTumourSite,data = df)
summary(coxmodel)
...
timeinc = 12
overallcox<-cph(Surv(SurvivalTimeMonth, Status==1)~AgeAtDiagnosis+Gender+PrimaryTumourSite+Morphology.Description, 
...
cancercox<-cph(Surv(SurvivalTimeMonth, StatusCancer==1)~AgeAtDiagnosis+Gender+PrimaryTumourSite+Morphology.Description, 
               x=T, y=T, surv=T, data = df, time.inc = timeinc)
...
r_fit<-ranger(Surv(SurvivalTimeMonth, Status==1)~AgeAtDiagnosis+Gender+PrimaryTumourSite+Morphology.Description,
              data = df2)
...
overallrpa <-rpart(Surv(SurvivalTimeMonth, Status)~AgeAtDiagnosis+Gender+PrimaryTumourSite+Morphology.Description,
                   data = df2,x=T,y=T)
summary(overallrpa)
overallrpa
